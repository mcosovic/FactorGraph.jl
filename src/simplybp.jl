...
function bps(H, b, v, MAXI, DAMP, BUMP, PROB, ALPH, MEAN, VARI, TIME)
    ...
    init = @elapsed begin
        ...
        m_fv, vi_fv, msr, vsr = factor_to_variable(m_vf, v_vf, m_fv, vi_fv, msr, vsr, Hi, Hr, bi, vi, Ii, Nli, -1, BUMP)
    end
    ...
    infe = @elapsed begin
        for i = 1:MAXI
            ...
            if i < DAMP
                m_fv, vi_fv, msr, vsr = factor_to_variable(m_vf, v_vf, m_fv, vi_fv, msr, vsr, Hi, Hr, bi, vi, Ii, Nli, i, BUMP)
            else
                m_fv, vi_fv, msr, vsr = dfactor_to_variable(m_vf, v_vf, m_fv, vi_fv, msr, vsr, Hi, Hr, bi, vi, Ii, Nli, ah1, ah2, i, BUMP)
            end
        end
    end
