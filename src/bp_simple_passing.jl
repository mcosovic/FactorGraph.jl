...
inference = @elapsed begin
    for i = 1:BUMP
        if i < DAMP
            ...
            Mfac_var, Wfac_var = factor_to_variable_damp(Mvar_fac, Vvar_fac, Mfac_var, Wfac_var, Mind, Vind, coeff,
                                                         coeffInv, row, Nind, row_colptr, to_var, alpha1, alpha2)
        end
        Mvar_fac, Vvar_fac = variable_to_factor(Mvar_fac, Vvar_fac, Mfac_var, Wfac_var,
    ...
    for i = (BUMP + 1):MAXI
        ...
        Mvar_fac = variable_to_factor_mean(Mvar_fac, Vvar_fac, Mfac_var, Wfac_var,
                                           Mdir, col, Nvar, col_colptr, to_fac)
    end
end
